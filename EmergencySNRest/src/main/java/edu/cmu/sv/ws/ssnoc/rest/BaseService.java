...
public class BaseService {
	...
	protected MessageDetailPO loadExistingUserStatus(long userId) {
		MessageDetailPO mpo = DAOFactory.getInstance().getMessageDetailDAO().findById(userId);
		if (mpo == null) {
			throw new UnknownUserException("No message associated with this userId");
		}

		return mpo;
	}
	
	protected MessageDetailPO findMessageById(long messageId) {
		MessageDetailPO mpo = DAOFactory.getInstance().getMessageDetailDAO().findByMessageId(messageId);
		if (mpo == null) {
			throw new UnknownUserException("No message associated with this messageId");
		}
		return mpo;
	}
}
